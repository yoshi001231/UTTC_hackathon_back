...
func (dao *TimelineDAO) FetchUserTimeline(userID string) ([]model.Post, error) {
	rows, err := dao.db.Query(`
		SELECT p.post_id, p.user_id, p.content, p.img_url, p.created_at, p.parent_post_id
		FROM posts p
		JOIN followers f ON p.user_id = f.following_user_id
...
func (dao *TimelineDAO) FetchUserPosts(userID string) ([]model.Post, error) {
	rows, err := dao.db.Query(`
		SELECT post_id, user_id, content, img_url, created_at, parent_post_id
		FROM posts
		WHERE user_id = ? AND deleted_at IS NULL
		ORDER BY created_at DESC
	`, userID)
	if err != nil {
		log.Printf("投稿一覧取得失敗 (user_id: %s): %v", userID, err)
		return nil, err
	}
	defer rows.Close()
