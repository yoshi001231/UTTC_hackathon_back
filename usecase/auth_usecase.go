...
func (uc *AuthUseCase) RegisterUser(userID, name, bio, profileImgURL string) (string, error) { // 修正: メソッド名をわかりやすく変更
	// バリデーション
	if userID == "" {
		return "", errors.New("[auth_usecase.go] user_id が無効: 必須項目")
	}
	if name == "" || len(name) > 50 {
		return "", errors.New("[auth_usecase.go] 名前が無効: 必須項目で50文字以内である必要がある")
	}
	if len(bio) > 160 {
		return "", errors.New("[auth_usecase.go] 自己紹介が無効: 160文字以内である必要がある")
	}
